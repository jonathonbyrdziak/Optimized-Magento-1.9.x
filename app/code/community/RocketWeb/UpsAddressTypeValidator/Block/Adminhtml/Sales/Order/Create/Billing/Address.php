...
class RocketWeb_UpsAddressTypeValidator_Block_Adminhtml_Sales_Order_Create_Billing_Address extends Mage_Adminhtml_Block_Sales_Order_Create_Billing_Address
{
	/**
     * Return header text
     *
        ...
        if(Mage::getModel('ups_address_validator/config')->isEnabled())
        {
	        ...
	        $_text .= $this->getLayout()->createBlock('adminhtml/widget_button')
	                ->setData(array(
	                    ...
	                    'onclick'=> 'return RocketWeb.Address.validate(\'order-billing_address\',\'' .Mage::helper('adminhtml')->getUrl('adminhtml/address/validate') . '\', \'order\');'
	                ))->setAfter("-")->toHtml();
	        return $_text;
        }
